...
***YOU MAY NOW USE IT WITH CAUTION AND REPORT ME ANY ENCOUNTERED TROUBLE***

***LET ME REMIND YOU I AM NOT A SOFTWARE ENGINEER BUT A PASSIONATE AUTODIDCT TINKERER INSTEAD***
...
GE-Proton seems to work the same way for all distros sincethe RTBS is the same (955) except for Linux Mint (938 with Tkg Proton) and Lubuntu (854) which used a different compatibility tool for the first one and requires the use of Wine and Windows Steam with Lutris Plateform to correctly access game management options. In case of Linux Mint, GE-proton was already known to be incompatible. Lubuntu seems to have problem to manage Linux Steam app since "Management" options can not be accessed so setting a compatibility tool is impossible and Steam must be installed with Wine and Steam Windows app to have access to all features. Moreover, custom grub wallpaper does not properly display when booting the system. With Pop!OS it does not display at all but it has no influence on the global package performance. For now, System Updater app does not work either with Linux Mint since it normally requires Snap installation for all distros and the dependance is broken in case of Linux Mint so testing were performed with a customized package build by removing "Snap" installation in the Sys_Config.sh script and the "snap refresh" instruction in the Sysupdater.sh script. This means that the programm is not compatible with Linux Mint for now and new conditions have to be set in the future update to remove Snap dependance from Linux Mint as well as Pop!OS and Zorin OS because snap is not native and many Ubuntu-based distros users dislike snap packages use.

All systems were able to access 1v1 ranked games within one hour. Lubuntu total time was not recorded because it requires a non standard Steam Installation and the use of Lutris plateform to complete the overall testing procedure task. Password feedback correctly works for all distros but we have to remind this option is already working by default. This option provides a great support for terminal-based programs which use "sudo" instructions and thus require the user password to be launched. Ubuntu 22.04.5 (t tot = 34 min) is the quickest to be set up but Xubuntu 24.04.1, Ubuntu 24.04.1 and Zorin OS 17.2 Core are very close (t tot are respectively 36, 37 and 37.5 min). Kubuntu and Pop!OS come after with a total testing procedure time recorded at 44.5 and 44.0 min which might be explained by many additionnal apps installed by default on the system. Linux Mint is last in line with 58 min but with a custom installation program without Snap installation. Thus, all IP1 indicator are above 100 except Lubuntu which can not be assessed since t7 was not recorded so was t tot. The IP2 indicator (ready to launch GE-proton patched Steam Platform within 30 min) is more restrictive and less RAM quantity dependant shows results higher than 100 for Ubuntu 22.04.5 and 24.04.1 as well as Xubuntu 24.04 and Zorin OS 17.2 core (respectively 127.7, 125.0, 125.0 and 117.6) but their values were slghly inferior to 100 for Pop!OS (93.8), Kubuntu 24.04 (90.9), Lubuntu (85.7) and Linux Mint (78.9). 

These results proves that the lightest system is not certainly the fastest to be set up. Indeed, since many OS optimization has to be removed and additionnal packages and libraries must be installed, the package installation and thus the testing procedure is longer even though the OS installation is quicker. Normally, Elementary OS was also scheduled to be tested in this study but preliminary tests showed that there were too many changes in the OS configuration while running BeLGE configuration scripts. Indeed, it took about 45 minutes just to run the Sys_Config.sh script and there were many changes in the OS which requires the user to choose between many different options so I decided to remove Elementary OS from further investigations. Nevertheless, it does not mean I can not recommand this OS for people with standard computer needs such as web browsing or office tools uses. Indeed, it remains both a light and good looking OS for people with few computering needs. 
       
* **Conclusion**

BeLGE appears to be compatible and effective on Ubuntu 22.04 and 24.04 as well as on its flavour Kubuntu and Xubuntu but also on Zorin OS and Pop!OS which are Ubuntu-based distros. Final Performance Test results are the same and demonstrate a free and friendly alternative to Microsft Windows for gaming purpose. Even though the laptop used for testing had many hardware problems (Skylake CPU performance loss are about 35% since Intel Downfall Vulnerability Security Patch and an unusable RAM slot out of two), the system was able to achieve a suitable performance score to access 1vs1 ranked games on Age of Empires 2 Definitive Edition but a bit too low to achieve ranked multiplayer games (955 instead of at least 1 000). Results are quite appreicable for a 9 year old CPU/GPU unit and another test session on MS Windows 10 on this hardware (Windows 11 is not compatible) should give us a better comparison of either improvements or limitations of the package. Unfortunatly, BeLGE isn't compatible with Linux Mint for now due to the lack of Snap support but some modifications on next version would easily fix that. It will remain impossible to use GE-Proton compatibility tool but Davidotek ProtonUp-Qt provides more tools and Lutris Gaming Platform is also available to fix some issues on native Steam Linux Debian package as well. In on hand, Lubuntu exhibits problems to access Game Management Options on Steam Linux and if it is possible to access the game through Steam Windows thanks to Lutris Platform RTBS is too low to access online ranked game (less than 900). In another hand, Lubuntu is not fitted for gaming purpose but for running systems with low hardware specifications thus it is not recommanded to use Lubuntu to play online video games but for office desktop purpose instead. Elementary OS is another friendly and good looking alternative system based on Ubuntu but during BeLGE configuration script there were too many system modifications to recommand its use on it. 
